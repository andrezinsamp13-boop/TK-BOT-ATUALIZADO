...
async function handleConfigModal(interaction) {
  ...
  if (section.startsWith('btn_edit_')) {
    ...
    if (wizard) {
      ...
      // Sessão expirada — re-renderiza pelo interaction atual
      const fakeInteraction = { update: (p) => interaction.editReply(p) };
      await showBtnEditor(fakeInteraction, btnId).catch(e => console.warn('[CONFIG MODAL] showBtnEditor fallback error:', e.message));
    }
  } else {
    ...
    if (fn) {
      const target = wizard ? wizard : interaction;
      await fn({ update: (p) => target.editReply(p) }).catch(e => console.warn('[CONFIG MODAL] section render error:', e.message));
    }
  }
}
